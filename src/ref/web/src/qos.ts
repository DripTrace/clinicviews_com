    ...
    const refreshIntervalId = setInterval(async () => {
        ...
        getStatsResult.forEach((item: any) => {
            switch (item.type) {
                ...
                case "candidate-pair":
                    qosStatsObj.RTD = Math.round(
                        (item.currentRoundTripTime / 2) * 1000
                    );
                    break;
                case "outbound-rtp":
...
const calculateStats = (qosStatsObj: QosStats): QosStats => {
    const rawNLR =
        (qosStatsObj.inboundPacketsLost * 100) /
            (qosStatsObj.inboundPacketsReceived +
                qosStatsObj.inboundPacketsLost) || 0;
    const rawJBN =
    ...
    return {
        ...
        MOSCQ: calculateMos(
            qosStatsObj.outboundPacketsLost /
                (qosStatsObj.outboundPacketsLost +
                    qosStatsObj.outboundPacketsSent)
        ),
    };
};
...
function calculateMos(packetLoss: number) {
    ...
    const r =
        93.2062077233 -
        95.0 * ((packetLoss * 100) / (packetLoss * 100 + bpl)) +
        4;
    let mos =
        2.06405 +
        0.031738 * r -

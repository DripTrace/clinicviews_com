...
const PatientRegistration: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert('Patient registered successfully! Please check your email for confirmation.');
      // Reset form
      setName('');
      setEmail('');
      setPhone('');
  ...
  return (
    <form onSubmit={handleSubmit} className={`max-w-md mx-auto ${isDarkMode ? 'text-[#D1E0EB]' : 'text-[#494949]'}`}>
      ...
      <div className="mb-4">
        <label htmlFor="pdf" className="block mb-2">Upload PDF Document</label>
        <input
          type="file"
          id="pdf"
          ref={fileInputRef}
      ...
      <div className="mb-4">
        <label className="flex items-center">
          <input
            ...
            onChange={(e) => setSuggestAppointment(e.target.checked)}
            className="mr-2"
          />
      ...
      >
        {isLoading ? 'Submitting...' : (suggestAppointment ? 'Register and Suggest Appointment' : 'Register')}
      </button>
    </form>
  );
};

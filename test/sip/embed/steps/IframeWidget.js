const { getNewWindowPromise } = require("../helpers");
...
export class IframeWidget {
    ...
    async loadElement() {
        await this._targetPage.waitForSelector(
            "iframe#rc-widget-adapter-frame",
            { timeout: 300000 }
        );
        await this._targetPage.waitForTimeout(3000);
        const iframeElement = await this._targetPage.$(
            "iframe#rc-widget-adapter-frame"
        );
    ...
    async waitForLoginPage() {
        await this._widgetIframe.waitForSelector(
            'button[data-sign="loginButton"]:not([disabled])'
        );
    }
    ...
    async clickLoginButtonToGetLoginWindow() {
        ...
        await this._widgetIframe.click(
            'button[data-sign="loginButton"]:not([disabled])'
        );
    ...
    async getLoginButtonText() {
        const loginText = await this._widgetIframe.$eval(
    ...
    async getDialButton() {
        const callBtn = await this._widgetIframe.$(
            'button[data-sign="callButton"]'
        );
        return callBtn;
    }
    ...
    async waitDialButtonEnabled() {
        await this._widgetIframe.waitForSelector(
            'div[data-sign="spinnerOverlay"]',
            { hidden: true }
        );
        await this.waitFor(
            'button[data-sign="callButton"]:not([disabled])',
            100000
        );
        const callBtn = await this._widgetIframe.$(
            'button[data-sign="callButton"]:not([disabled])'
    ...
    async clickNavigationButton(label) {
        await this.waitFor('ul[data-sign="navigationBar"]');
        const tab = await this._widgetIframe.$(
            `.MuiListItem-root[role="tab"][data-sign="${label}"]`
        );
        if (tab) {
            await this._widgetIframe.click(
                `.MuiListItem-root[role="tab"][data-sign="${label}"]`
        ...
        await this._widgetIframe.click(
            '.MuiListItem-root[role="tab"][data-sign="More"]'
        );
    ...
    async moreButtonExist() {
        const moreButton = await this._widgetIframe.$(
            'button[data-sign="More"]'
        );
        return !!moreButton;
    }
    ...
    async clickSubTab(label, path) {
        ...
        const hasHidden = await this._widgetIframe.$eval(
            `.MuiTab-root[data-sign="${label}"]`,
            (element) => {
        ...
        if (hasHidden) {
            await this._widgetIframe.click(
                ".MuiTabs-flexContainer button[data-tab-more-button]"
            );
            await this._widgetIframe.waitForTimeout(1000);
    ...
    async clickNavigationSubMenu(label) {
        ...
        await this._widgetIframe.click(
            `.MuiMenuItem-root[data-sign="${label}"]`
        );
    }
    ...
    async getCallItemList() {
        ...
        const callItems = await this._widgetIframe.$$(
            'div[data-sign="calls_item_root"]'
        );
        return callItems;
    }
    ...
    async getNoCallsText() {
        ...
        const noCallsText = await this._widgetIframe.$eval(
            ".CallsListPanel_noCalls",
            (el) => el.innerText
        );
        return noCallsText;
    }
    ...
    async getMessageList(type) {
        ...
        await this._widgetIframe.waitForTimeout(500); // for render
        const voicemailItems = await this._widgetIframe.$$(
            `div[data-sign="${type}"]`
        );
    ...
    async getNoMessageText() {
        ...
        const noMessageText = await this._widgetIframe.$eval(
            'p[data-sign="noMatch"]',
            (el) => el.innerText
        );
    ...
    async getMessageAllTabText() {
        ...
        const allTabText = await this._widgetIframe.$eval(
            'button[data-sign="All"]',
            (el) => el.innerText
        );
        return allTabText;
    }
    ...
    async getContactSearchInput() {
        ...
        const searchInput = await this._widgetIframe.$(
            'div[data-sign="contactsSearchInput"] input'
        );
        return searchInput;
    }
    ...
    async getEULAText() {
        ...
        const elua = await this._widgetIframe.$eval(
            'a[data-sign="eula"]',
            (el) => el.innerText
        );
        return elua;
    ...
    async clickSettingSection(label) {
        await this.waitFor(".SettingsPanel_root");
        await this._widgetIframe.click(
            `div[data-sign="${label}"][role="button"]`
        );
    }

    async getSettingSection(label) {
        await this.waitFor(".SettingsPanel_root");
        const section = await this._widgetIframe.$(
            `div[data-sign="${label}"][role="button"]`
        );
        return section;
    }
    ...
    async getHeaderLabel() {
        ...
        const text = await this._widgetIframe.$eval(
    ...
    async getSMSRecipientNumber() {
        ...
        const value = await this._widgetIframe.$eval(
            'input[name="receiver"]',
            (el) => el.value
        );
        return value;
    ...
    async typeSMSRecipientAndText({ recipientNumber, text }) {
        await this._widgetIframe.type(
            'input[name="receiver"]',
            recipientNumber
        );
        await this._widgetIframe.waitForTimeout(1000);
        await this._widgetIframe.type(
            'textarea[data-sign="messageInput"]',
            text
        );
    }
    ...
    async getLastTextAtConversation() {
        ...
        const messages = await this._widgetIframe.$$eval(
            'div[data-sign="OutboundText"]',
    ...
    async getServiceNameInAuthorizationSettings() {
        ...
        const text = await this._widgetIframe.$eval(
            'div[data-sign="thirdPartyAuthSetting"] .RcListItemText-primary',
            (el) => el.innerText
        );
        return text;
    }
    ...
    async getContactFilters() {
        ...
        const filters = await this._widgetIframe.$$eval(
            'div[role="tablist"] button:not([data-tab-more-button])',
            (els) => els.map((el) => el.innerText)
        );
        return filters;
    }
    ...
    async getContactNames() {
        const texts = await this._widgetIframe.$$eval(
    ...
    async getTabHeader() {
        const text = await this._widgetIframe.$eval(
            'h6[data-sign="navigationHeaderTitle"]',
            (el) => el.innerText
        );
        return text;
    }

...
//         const voicemailItems = await widgetIframe.getMessageList(
...
//             await widgetIframe.getSMSRecipientInputPlaceholder();
//         expect(recipientPlaceholder).toEqual("Enter name or number");
//         const text = `text ${Date.now()}`;
//         await widgetIframe.typeSMSRecipientAndText({
//             recipientNumber: __TEST_SMS_RECEIVER_NUMBER__,
//             text,
//         });
//         await widgetIframe.clickSMSSendButton();
//         const lastTextInConversation =
//             await widgetIframe.getLastTextAtConversation();
...
//         await widgetIframe.waitForNavigation();
//         const smsLink = await page.$(
//             'a[href="sms:+12345678902?body=test_sms"]'
//         );
//         await smsLink.evaluate((l) => l.click());
//         const recipientNumber = await widgetIframe.getSMSRecipientNumber();
...
//         });
//         const serviceName =
//             await widgetIframe.getServiceNameInAuthorizationSettings();
//         expect(serviceName).toEqual("TestService");
//         await widgetIframe.clickSettingSection("logging");
//         const openLoggingPageAfterCall = await widgetIframe.getSettingSection(
//             "thirdPartySettings-openLoggingPageAfterCall"
//         );
//         expect(!!openLoggingPageAfterCall).toEqual(true);
//         const contactsSection =
//             await widgetIframe.getSettingSection("contacts");
...
//                         unreadCount: 1,
//                         iconUri:
//                             "https://github.com/ringcentral/ringcentral-embeddable/assets/7036536/d23f7dbf-fcf7-4011-a538-16adfba06a66",
//                         activeIconUri:
//                             "https://github.com/ringcentral/ringcentral-embeddable/assets/7036536/10a1edf9-9837-4dec-8950-7ec81b14be4d",
//                         schema: {
//                             type: "object",
//                             properties: {
//                                 search: {
...
//                                             title: "Opportunity 2",
//                                             description:
//                                                 "This is a description message 2",
//                                             meta: "4/15",
...
//                                 type: "string",
//                                 description:
//                                     "Please authorize the CRM to use this feature.",
//                             },
//                             someTitle: {
//                                 type: "string",
...
const hasUserLoginInfo = global.__JWT_TOKEN__;
if (!hasUserLoginInfo) {
    console.log("THOSE TESTS ARE SKIPPED BECAUSE NO USER LOGIN INFO PROVIDED");
}
const conditionalTest = !hasUserLoginInfo ? test.describe.skip : test.describe;

conditionalTest("widget page test", () => {
    let widgetIframe;
    ...
    test("should have dial button enabled", async () => {
        await widgetIframe.waitDialButtonEnabled();
        const noTimeout = true;
        expect(noTimeout).toEqual(true);
    });

    test("should goto calls page successfully", async () => {
        await widgetIframe.clickSubTab("Calls", "/history");
        const callItems = await widgetIframe.getCallItemList();
        const noCallsText = await widgetIframe.getNoCallsText();
        const isNoCalls = noCallsText === "No results found.";
    ...
    test("should goto voicemail page successfully", async () => {
        await widgetIframe.clickSubTab("Voicemail", "/messages/voicemail");
        const voicemailItems = await widgetIframe.getMessageList(
            "VoiceMailMessageItem"
        );
    ...
    test("should goto recordings page successfully", async () => {
        await widgetIframe.clickSubTab("Recordings", "/history/recordings");
        const callItems = await widgetIframe.getCallItemList();
        const noCallsText = await widgetIframe.getNoCallsText();
        const isNoCalls = noCallsText === "No results found.";
    ...
    test("should goto fax page successfully", async () => {
        await widgetIframe.clickNavigationButton("Fax");
        const tabHeaderText = await widgetIframe.getTabHeader();
        expect(tabHeaderText).toEqual("Fax");
    });

    test("should goto text page successfully", async () => {
        await widgetIframe.clickNavigationButton("Text");
        const tabHeaderText = await widgetIframe.getTabHeader();
        expect(tabHeaderText).toEqual("Text");
    });

    test("should goto Compose Text page successfully", async () => {
        await widgetIframe.clickComposeTextIcon();
        const recipientPlaceholder =
            await widgetIframe.getSMSRecipientInputPlaceholder();
        expect(recipientPlaceholder).toEqual("Enter name or number");
    ...
    test("should goto contacts page successfully", async () => {
        await widgetIframe.clickNavigationButton("Contacts");
        await widgetIframe.waitForTimeout(1000);
        const tabHeaderText = await widgetIframe.getTabHeader();
        expect(tabHeaderText).toEqual("Contacts");
    ...
    test("should goto settings page successfully", async () => {
        await widgetIframe.gotoSettingsPage();
        const title = await widgetIframe.getTabHeader();
        expect(title).toEqual("Settings");
    });

    test("should goto region setting page successfully", async () => {
        await widgetIframe.gotoSettingsPage();
        await widgetIframe.waitForTimeout(1000);
        await widgetIframe.clickSettingSection("region");
        const headerLabel = await widgetIframe.getHeaderLabel();
    ...
    test("should goto audio setting page successfully", async () => {
        await widgetIframe.gotoSettingsPage();
        await widgetIframe.waitForTimeout(1000);
        await widgetIframe.clickSettingSection("audio");
        const headerLabel = await widgetIframe.getHeaderLabel();
    ...
    test("should goto calling setting page successfully", async () => {
        await widgetIframe.gotoSettingsPage();
        await widgetIframe.clickSettingSection("calling");
        const headerLabel = await widgetIframe.getHeaderLabel();
        expect(headerLabel).toEqual("Calling");
    ...
    test("should goto Compose Text page when click SMS link", async () => {
        await widgetIframe.waitForNavigation();
        const smsLink = await page.$('a[href="sms:+12345678901"]');
        await smsLink.evaluate((l) => l.click());
        const recipientNumber = await widgetIframe.getSMSRecipientNumber();
    ...
    test("should goto Compose Text page when click SMS link with body", async () => {
        await widgetIframe.waitForNavigation();
        const smsLink = await page.$(
            'a[href="sms:+12345678902?body=test_sms"]'
        );
                                ...
                                items: [
                                    {
                                        ...
                                        type: "externalLink",
                                        name: "Document",
                                        uri: "https://ringcentral.github.io/ringcentral-embeddable/docs/",
                                    },
                                    {
                                        id: "feedback",
                                        type: "button",
    ...
    test("should register customized page and tab successfully", async () => {
        await page.evaluate(() => {
            const iframe = document.querySelector(
                "#rc-widget-adapter-frame"
            ).contentWindow;

...
// Wrapper for handling WebSocket events
const handleWebSocketEvents = (
    ws: any,
    eventHandlers: Record<string, (frame: { payload: string }) => void>
) => {
    ws.on("framesent", (frame: { payload: string }) => {
        if (eventHandlers["framesent"]) {
            eventHandlers["framesent"](frame);
        }
    });

    ws.on("framereceived", (frame: { payload: string }) => {
        if (eventHandlers["framereceived"]) {
            eventHandlers["framereceived"](frame);
        }
    });
};
...
const login = async (
    ...
    // Here we assume wsHandler will handle events in a custom way
...
test("home page", async ({ context }) => {
    ...
    const receiverPage = await login(
        context,
        process.env.RC_WP_RECEIVER_JWT_TOKEN!,
        () => {}
    );
    await receiverPage.screenshot({
        path: "screenshots/receiver-logged-in.png",
    });
    ...
    const r = await rc.restapi().account().extension().phoneNumber().get();
    const receiverPhoneNumber = r.records!.filter((p) => p.primary === true)[0]!
        .phoneNumber!;
    await rc.revoke();
    await callerPage.fill('input[name="number"]', receiverPhoneNumber);
    await callerPage.click("#btn-call");
    ...
    await expect(receiverPage.locator("text=Call In Progress")).toBeVisible();
    await receiverPage.screenshot({
        path: "screenshots/receiver-answered.png",
    });
    ...
    await login(
        ...
        (ws: any) => {
            handleWebSocketEvents(ws, {
                framesent: (frame) => {
                    const parsed = sip.Core.Parser.parseMessage(
                        frame.payload,
                        logger
                    );
                    ...
                    if (parsed!.method === "REGISTER") {
                        expect(
                            parsed!.headers["P-Custom-Header"].length
                        ).toEqual(1);
                        expect(
                            parsed!.headers["P-Custom-Header"][0].raw
                        ).toEqual("CustomValue");
                        wsHandled = true;
                    }
                },
...
test("send client id during register if set", async ({ context }) => {
    ...
    await login(context, process.env.RC_WP_CALLER_JWT_TOKEN!, (ws: any) => {
        handleWebSocketEvents(ws, {
            framesent: (frame) => {
                const parsed = sip.Core.Parser.parseMessage(
                    frame.payload,
                ...
                if (parsed!.method === "REGISTER") {
                    expect(parsed!.headers["Client-Id"].length).toEqual(1);
                    expect(parsed!.headers["Client-Id"][0].raw).toEqual(
                        process.env.RC_WP_CLIENT_ID!
                    );
                    wsHandled = true;
                }
            },
...
test("refresh frequency setting", async ({ context }) => {
    test.setTimeout(70000);
    let firstRegisterAt: number | undefined;
    let wsHandled = false;
        ...
        (ws: any) => {
            handleWebSocketEvents(ws, {
                framesent: (frame) => {
                    ...
                    if (
                        parsed.method === "REGISTER" &&
                        typeof firstRegisterAt !== "undefined"
                    ) {
                        expect(Date.now() - firstRegisterAt).toBeLessThan(
                            30000
                        );
                        wsHandled = true;
                    }
